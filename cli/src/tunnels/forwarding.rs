...
use super::{
	...
	protocol::{self, forward_singleton::PortList},
...
impl PortForwardingReceiver {
	...
	pub async fn apply_to(&mut self, log: log::Logger, mut tunnel: ActiveTunnel) {
		let mut current = vec![];
...
pub async fn client(args: SingletonClientArgs) -> Result<(), std::io::Error> {
	...
	let forward = async move {
		while port_requests.changed().await.is_ok() {
			...
			if let Err(e) = r {
				error!(log, "failed to set ports: {:?}", e);
			}
		}
	};
...
pub async fn server(
	...
	let forward = async move {
		while port_requests.changed().await.is_ok() {
			forward_tx.set_ports(port_requests.borrow().clone());
		}
	};

	tokio::select! {
		_ = forward => Ok(()),
		_ = forward_rx.apply_to(log.clone(), tunnel) => Ok(()),
		r = serve_singleton_rpc(server, rpc.build(log), shutdown_rx) => r,
	}
}

async fn serve_singleton_rpc(
	mut server: SingletonServer,
	dispatcher: RpcDispatcher<JsonRpcSerializer, SingletonServerContext>,
	shutdown_rx: Barrier<ShutdownSignal>,
) -> Result<(), CodeError> {
	...
	loop {
		...
		let main_context = dispatcher.context();
		let mut my_dispatcher = dispatcher.clone();

		my_dispatcher.set_context(SingletonServerContext {
			log: main_context.log.clone(),
			// handle gets cloned so each client has its own handle and list of ports:
			handle: main_context.handle.clone(),
		});
		tokio::spawn(async move {
			let _ = start_json_rpc(my_dispatcher, read, write, (), shutdown_rx).await;
		});
	}
}
